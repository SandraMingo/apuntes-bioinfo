...
La conclusión es que se debe dejar la pendiente, pero se puede ignorar la variable sexo ya que no hay evidencia de que ese coeficiente sea distinto de 0 y, por tanto, afecte al modelo.

\subsection{Comparación formal de moelos}
...
Se trata de modelos, por lo que podemos, y debemos, comprobar algunos de sus supuestos básicos. 

En general, para los modelos lineales (ANOVAs, regresiones, etc) queremos comprobar:
\begin{itemize}
