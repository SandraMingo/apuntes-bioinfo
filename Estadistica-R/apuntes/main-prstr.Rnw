...
Una tercera diferencia es que la mayoría de los procedimientos automatizados como \code{step} con AIC añadirá o eliminará una "variable completa". Sin embargo, con \code{Anova} (y \code{glht}, y otros) puede probar hipótesis específicas de su interés, incluyendo hipótesis en las que, por ejemplo, la media de dos niveles de un factor es igual a la tercera, etc.

Por último, tenga en cuenta que \code{anova} sólo debe utilizarse para comparar modelos anidados (incluso si \code{Anova} y otros puede probar hipótesis de interés para usted). \code{step} seguirá las reglas de comparación codiciosas para pasar de un modelo a otro, pero si insiste podría mirar el AIC de una gran colección de modelos (o todos los modelos posibles para un conjunto de variables) y comparar entre modelos aunque no sean versiones anidadas entre sí. De nuevo, hacer esto utilizando el AIC podría ser sensato porque el objetivo es la predicción, no la comprobación de hipótesis.
...
Como consecuencia de lo anterior, por supuesto, a veces utilizar \code{step} es algo que no tendría sentido y que ni siquiera se plantearía, por ejemplo en un experimento limpio y claro con dos factores: se quiere utilizar un ANOVA, no hace falta \code{step}. Y, del mismo modo, con miles de variables posibles, ejecutar miles de pruebas de modelo manualmente no tendría sentido si está intentando construir un buen modelo predictivo; utilice un procedimiento que intente encontrar el mejor modelo desde el punto de vista de la predicción.
