...
\advance\cftsubsecnumwidth 0.9em\relax
...
Supuesto & Gráfico clave & Indicador de problemas \\ \hline \hline
Linealidad & Residuals vs Fitted & Patrones curvados o no aleatorios \\ \hline
Independencia de residuos & Residuals vs Fitted & Residuos no distribuidos aleatoriamente\\ \hline
Normalidad de residuos & Normal Q-Q & Puntos alejados de la línea diagonal\\ \hline
Homocedasticidad & Scale-Location & Aumento o disminución sistemático de la dispersión \\ \hline
Ausencia de observaciones influyentes & Residuals vs Leverage & Puntos fuera de Cook's Distance
\end{tabular}
\end{table}

%02/12 - Ramón
La función \code{ancova} solo se utiliza para representar gráficos. La tabla de resultados utiliza la suma de cuadrados secuencial. Desde el punto de vista del modelo que va a analizar, es mucho menos flexible que \code{Anova}. La ventaja de utilizar \code{Anova} es que se puede utilizar para modelos lineales de complejidad arbitraria que pueden tener variables discretas, continuas, interacción, etc sin limitación.

\subsection{Diagnóstico: ejemplo con regresión y modelo ANCOVA}
...
\subsection{Diagnóstico: más ejemplos de varianza no constante}
...
\subsection{Diagnóstico: un par de ejemplos de modelos de ANOVA que están bien}
...
Se saca la distancia de Cook directamente para poder determinarla. Este modelo presenta un problema similar al anterior. El Q-Q plot es un poco feo, pero lo que nos alera es Residuals vs Fitted. El plot diagnóstico nos ayuda para identificar un problema estructural. La línea roja (las medias de los grupos) están sistemáticamente desviadas de 0, estando dos por encima y dos por debajo. En este caso, lo que faltó fue el modelo con interacción.
...
\subsection{Diagnóstico: otras cuestiones}
\begin{itemize}
\item Con el paquete \code{car} se puede generar un Q-Q plot extendido.
\item Los gráficos "Componente+Residual" (o residual parcial) nos permiten examinar, en modelos con múltiples regresores, las desviaciones de la linealidad y podrían sugerir la transformación apropiada. Los gráficos "CERES" son una variación de los gráficos "Componente + Residual" que funcionan bien incluso si las relaciones son fuertemente no lineales. Ambos también están disponibles en \code{car}.
\item Diversos diagnósticos relacionados con dfbetas nos permiten identificar observaciones influyentes en términos específicos del modelo.
\item Los factores de inflación de la varianza nos ayudan a detectar posibles problemas causados por colinealidad (correlaciones entre variables independientes).
\item Los gráficos de variables añadidas son especialmente útiles en problemas de regresión múltiple con múltiples variables independientes; pueden ayudar a identificar puntos influyentes (que se enmascaran fácilmente con múltiples variables) y también pueden ayudar a intentar encontrar una buena relación funcional (pero Componente+Residual son más útiles en este caso). De nuevo, disponible en \code{car}.
\item También se dispone de diversas pruebas numéricas y diagnósticos (por ejemplo, pruebas de no linealidad o de homocedasticidad).

\item ¿Qué ocurre si los diagnósticos detectan un problema? Los procedimientos habituales son asegurarse de que el modelo es correcto y, posiblemente, transformar la respuesta o algunos de los predictores, y tal vez utilizar modelos más complejos (por ejemplo, para modelar la varianza, etc.). Pero \textbf{antes} de ajustar un modelo, hay que pensar detenidamente cuál es el modelo biológico apropiado del fenómeno. Eso podría dictar, por ejemplo, transformaciones razonables a priori de la respuesta o los predictores, o cuál debe ser la forma funcional. (Por ejemplo, modelar la tasa metabólica como una función de la masa corporal fue una mala idea; existen hay muchos argumentos de peso que sugieren que lo mejor es una relación logarítmica).

\end{itemize}

\section{Selección de modelo y variables}
\subsection{Selección de modelos}
Lo primero que hay que tener en cuenta es que los procedimientos basados exclusivamente en criterios de significación estadística pueden seleccionar variables «estadísticamente importantes» (según una definición adecuada de «importante»), pero no tienen por qué ser las más relevantes desde el punto de vista biológico, causal, etc. Se trata de un resumen muy rápido de algunas de las razones por las que ajustamos modelos estadísticos y, a continuación, quizá llevemos a cabo la selección de variables y/o modelos:

\begin{itemize}
\item \textbf{Interpretación/comprensión:} ¿por qué las cosas son así? ¿Qué factores son relevantes para un proceso y por qué? Se trata de obtener una comprensión científica en el sentido de «descubrir la verdad». ¿Es la falta de sueño la causa de lo que sea? ¿O son las diferencias en la exposición a la luz? O ... En esto consiste a menudo gran parte (¡¡no toda!!) la modelización científica.

\item \textbf{Predicción:} predecir una variable a partir de otras. Esto puede no tener nada que ver con la intervención (que es lo que suele perseguir la inferencia causal) o con la comprensión de un proceso.
...
\item \textbf{Inferencia causal, intervención:} ¿qué ocurriría en Y si manipuláramos X? Por ejemplo, si consiguiéramos que la gente comiera legumbres dos veces por semana, ¿cuántos cánceres de colon podríamos prevenir?
\end{itemize}
...
Por supuesto, la intervención tampoco está totalmente desvinculada del éxito de la predicción. Los elementos de los modelos que desempeñan un papel en la predicción meteorológica y climatológica también están presentes en los modelos causales que se utilizan para orientar (o intentar orientar, desgraciadamente sin mucho éxito) las acciones para evitar el desastre del cambio climático; aquí nos preguntamos "¿qué variables debemos cambiar, y a qué valores, para conseguir este resultado o evitar este otro?"
...
Una cuidadosa comparación de modelos, por ejemplo usando algo como \verb@ anova(model1, model2) @, como hemos visto podría ser una buena idea. Observa, de nuevo, que \verb@ anova(modelo1, modelo2) @ se trata de comparar modelos.

\subsection{Selección de modelo utilizando AIC y \code{step}}
Si realmente se necesitan procedimientos automatizados o semiautomatizados, entonces las estrategias razonables utilizan criterios de comparación de modelos como el AIC (por ejemplo, con la función \code{step}) son mucho más sensatas.  Pero el enfoque, ahora, es diferente: estamos haciendo \textbf{selección de modelo}, no la selección de variables. Y estamos tratando de lograr un objetivo diferente: estamos tratando de encontrar el mejor modelo para hacer predicciones. En otras palabras, estamos tratando de encontrar el modelo que, cuando se aplica a los nuevos datos, dará las mejores predicciones.

\subsubsection{Introducción a AIC}
...
\textbf{Ejemplo con tiradas de una moneda}
Supongamos que lanzamos una moneda y obtenemos cara ($x=1$). Si $\theta$ es la probabilidad de obtener cara y el valor verdadero de $\theta$ es 0,5, entonces la probabilidad de observar $x=1$ es:
$$P(x = 1 | \theta = 0,5) = 0,5$$
...
$$P(x = 2 | \theta = 0,7) = \begin{pmatrix} 3 & 2 \end{pmatrix} \cdot 0,7^2 \cdot (1 - 0,7)^1 = 3 \cdot 0,49 \cdot 0,3 = 0,441$$
...
Esencialmente, $L(\theta)$, es la probabilidad de los datos bajo el parámetro $\theta$ (es mejor pensar en la densidad conjunta, ya que incluye, entonces, distribuciones continuas). Para un conjunto de datos, podemos encontrar el valor de $\theta$, los parámetros, que hace que la probabilidad sea tan grande como puede ser; esta sería la estimación de máxima verosimilitud del parámetro(s) $\theta$.  Así que en la expresión anterior, $\hat{L}$ es el valor de la probabilidad cuando se evalúa con los parámetros que hacen que la probabilidad sea lo más grande posible, es decir, en el valor(es) que hacen que los datos sean lo más probable posible. En un ejemplo de regresión, éste sería el valor de la probabilidad cuando los coeficientes de regresión (y el intercepto) toman los valores estimados por el software de regresión.

El cálculo AIC es el logaritmo de la verosimilitud penalizado por el número de parámetros. En otras palabras, el AIC muestra el equilibrio entre un buen ajuste (a los datos actuales) y el número de parámetros: los modelos con muchos parámetros podrían ajustarse mejor a los datos (es decir, tener mayor verosimilitud), pero un ligero aumento de la verosimilitud podría no justificar algunos de esos parámetros. Así que el AIC dice algo así como "una mayor verosimilitud es buena, pero no te dejes llevar añadiendo demasiados parámetros". Con demasiados parámetros, el modelo podría ser demasiado complejo no para los datos actuales (la probabilidad ha aumentado), sino para hacer buenas predicciones en otros conjuntos de datos. Demasiados parámetros aumentan el riesgo de \textbf{sobreajuste}.

Tres características clave de la AIC:
\begin{enumerate}
\item El AIC (bajo ciertos supuestos) es una estimación insesgada de la precisión predictiva del modelo: la probabilidad esperada del modelo cuando se aplica a nuevos datos.
\item El AIC mide (de nuevo, bajo ciertos supuestos) lo cerca de los valores reales que están las predicciones del modelo. En este caso, la «proximidad» se evalúa mediante la divergencia de Kullback-Leibler, una medida de distancia entre distribuciones de probabilidad.
\item El AIC es equivalente a la validación cruzada «leave-one-out» para (muchos) modelos lineales (incluida la regresión lineal y los efectos mixtos). En un conjunto de observaciones, se deja una fuera y, con las demás, se calcula la predicción y se compara con el valor observado que se había dejado fuera. 
\end{enumerate}

\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.40\paperwidth,keepaspectratio]{figs/aic-otsuka.png}
    \caption{AIC as estimate of expected likelihood.  From J.\ Otsuka (2023), \textit{Thinking about statistics. The philosophical foundations.}, Routledge, Figure 4.2, p.~116. Se quiere ajustar un modelo lineal (sin intercepto en este caso). De la misma población se busca obtener más datos y hacer predicciones con la recta concreta. Esto se repite varias veces y se calcula la media. De la misma población se sacan nuevos datos y se calcula la recta, esta vez con pendiente diferente. Se generan predicciones y se obtiene la verosimilitud. AIC da la verosimilitud media, diciendo la verosimilitud de observaciones nuevas con el procedimiento de calcular el modelo.}
  \end{center}
\end{figure}

Supongamos que tenemos varios modelos y medimos el AIC de cada uno. La selección de modelos mediante el AIC nos dirá que prefiramos el modelo con el mayor AIC. (Nota: mayor si definimos AIC como arriba; deberíamos preferir el modelo con el AIC más pequeño si definimos AIC como
$AIC = -2 \ln(\hat{L}) + 2\ k$).
...
\begin{enumerate}
\item  El modelo así seleccionado (es decir, el modelo con el mejor AIC) no tiene por qué ser el modelo que tiene "todos y cada uno de los parámetros que realmente afectan al resultado": los "modelos más pequeños", los modelos que no incluyen algunos parámetros, podrían funcionar mejor. 

Para repetirlo: para un tamaño de muestra determinado, el mejor modelo según el AIC podría indicar que NO incluya una variable que realmente tenga un efecto (que sea "significativa"). No todas las variables que "realmente tienen un efecto" mejoran necesariamente el rendimiento predictivo; del mismo modo, si la relación es realmente ligeramente cuadrática, añadir el parámetro del término cuadrático no mejora necesariamente el rendimiento predictivo. Sí, esto significa que los "modelos verdaderos" (en términos de variables, parámetros, etc.) no son necesariamente los mejores modelos para el rendimiento predictivo, y que los modelos más simples suelen ser mejores. 

\item El tamaño (el número de parámetros) del modelo seleccionado depende del tamaño de la muestra: con muestras de mayor tamaño, a menudo podemos permitirnos ajustar modelos más grandes y, por tanto, con muestras de mayor tamaño, el AIC suele seleccionar modelos más grandes. (Más grande puede significar «más variables» o «más parámetros» si incluimos, por ejemplo, relaciones no lineales, etc.).

\end{enumerate}

\subsubsection{Selección de modelo utilizando AIC}
AIC no requiere que los modelos estén encajados. En R, la función \code{step} utiliza AIC, empezando con el modelo más grande a ajustar. Esta función intenta quitar términos y ver cuál de ellos, al quitarlo, mejora más AIC (se minimiza). En pasos posteriores, puede quitar términos o volver a añadirlos para comparar AIC.
...
\subsection{Diferencias entre selección de modelos utilizando AIC y comparación de modelos mediante \code{anova} (y testeo de hipótesis mediante \code{Anova})}
La principal diferencia es que cuando comparamos modelos utilizando la función \code{anova}, o examinamos cada uno de los términos de un modelo ajustado con, por ejemplo, la función \code{Anova}, estamos realizando pruebas de hipótesis estadísticas. 
Por el contrario, el criterio AIC no se utiliza para realizar pruebas de hipótesis, sino que es un criterio relacionado con el rendimiento predictivo; por lo tanto, cuando llevamos a cabo la selección de modelos utilizando el AIC, intentamos encontrar el mejor modelo, donde «mejor» significa «mejor desde el punto de vista de la predicción».

Una segunda diferencia es que cuando utilizamos \code{anova} y \code{Anova} para comparar modelos o evaluar la significación de diferentes variables, generalmente no lo hacemos (no deberíamos hacerlo) con decenas o cientos de modelos y variables. Estamos probando algunas hipótesis específicas y lo hacemos "manualmente". En cambio, utilizando la selección de modelos con AIC (o criterios similares) el procedimiento se ejecutará automáticamente y podrá comparar potencialmente cientos de modelos.

Una tercera diferencia es que la mayoría de los procedimientos automatizados como \code{step} con AIC añadirá o eliminará una "variable completa". Sin embargo, con \code{Anova} (y \code{glht}, y otros) se puedne probar hipótesis específicas de nuestro interés, incluyendo hipótesis en las que, por ejemplo, la media de dos niveles de un factor es igual a la tercera, etc.

Por último, \code{anova} sólo debe utilizarse para comparar modelos anidados (incluso si \code{Anova} y otros puede probar hipótesis de interés). \code{step} seguirá las reglas de comparación codiciosas para pasar de un modelo a otro, pero se podría mirar el AIC de una gran colección de modelos (o todos los modelos posibles para un conjunto de variables) y comparar entre modelos aunque no sean versiones anidadas entre sí. De nuevo, hacer esto utilizando el AIC podría ser sensato porque el objetivo es la predicción, no la comprobación de hipótesis.

Simplificando un poco las cuestiones, la comprobación de hipótesis y la construcción de modelos utilizando herramientas como \code{anova} y \code{Anova} son cosas que se hacen cuando se quiere comprender un fenómeno, mientras que la selección de modelos utilizando criterios como AIC es lo que se hace cuando se quiere construir modelos con el mejor rendimiento predictivo.

Como consecuencia de lo anterior, por supuesto, a veces utilizar \code{step} es algo que no tendría sentido y que ni siquiera se plantearía, por ejemplo en un experimento limpio y claro con dos factores: se quiere utilizar un ANOVA, no hace falta \code{step}. Y, del mismo modo, con miles de variables posibles, ejecutar miles de pruebas de modelo manualmente no tendría sentido si se está intentando construir un buen modelo predictivo; utiliza un procedimiento que intente encontrar el mejor modelo desde el punto de vista de la predicción.

\newpage
\subsection{Un modelo largo de pemax como ejemplo}
...
%09/12 - Ramón
\chapter{Elección de covariantes, interpretar coeficientes e inferencia causal}
...
"(...) efectos causales en
poblaciones, es decir, \textbf{cantidades numéricas} que miden los cambios en la distribución de un resultado bajo \textbf{intervenciones} diferentes."
...
\section{Grafos, DAGs y notación}
...
\section{Ejemplo introductorio del ajuste por covariantes de causa común: colesterol, ejercicio y edad}
...
\begin{figure}[h!]
  ...
  \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{figs/chol-ex1-crop}% first figure itself
%       \caption{first figure}
    \end{minipage}\hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=0.9\textwidth]{figs/chol-ex2-crop} %second figure itself
...
Debemos ajustar (o controlar) por Edad, la causa común del Ejercicio y la Edad: si no ajustamos por Edad, la estimación del efecto del Ejercicio sobre el Colesterol está confundida por la Edad que tiene efectos tanto sobre el Ejercicio como sobre el
Cuando se trata de variables categóricas, este patrón. Cuando la asociación de dos variables cambia, o incluso invierte su signo cuando tenemos en cuenta otra, también se denomina paradoja de Simpson. La edad es un \textbf{confundador}.

\section{Ajuste por covariantes: hongos y la variable post-tratamiento}
Se realiza un experimento para evaluar los efectos de un tratamiento antifúngico (Tratamiento) sobre el tamaño final de la planta (Tamaño). También se mide la cantidad de hongos (variable posterior al tratamiento, Fungus). Dado que la calidad de las parcelas varía, lo que podría afectar al tamaño final de las plantas, se ha incluido una variable (o variables) Parcela para medir la calidad de la parcela (pretratamiento, aunque esto es irrelevante, ya que esta variable, o variables, no se ven afectadas por el tratamiento). El DAG se muestra en la Fig. \ref{fig:plot_fungus_1}.
...
Qué ocurre con los hongos: no queremos ajustarlo, ya que el ajuste por hongos nos impediría estimar el efecto del tratamiento: El tratamiento afecta al tamaño de las plantas a través de los hongos. Una vez que sabemos acerca de los hongos, el tratamiento no dice nada sobre el tamaño. Se puede decir que hongo "apantalla" el tamaño (en inglés \textit{screens off}), ya que al condicionar hongo, hagas lo que hagas en tratamiento, no ves los efectos en el tamaño.

Incluso si la relación real es como la mostrada en \ref{fig:plot_fungus_2}, no querríamos utilizar Fungus como covariante. Tratamiento no es independiente de tamaño dado Fungus, pero Fungus media la relación. Añadiendo Fungus en el modelo estadístico, no se estaría estimando el efecto total de tratamiento en tamaño.
...
En la figura \ref{fig:fungus_collider}, Fungus es descendiente tanto de U como de Treatment. Esto se llama un \textbf{collider}. Condicionar a Fungus nos llevará a asociar Tratamiento y humedad (U), incluso cuando en realidad son independientes el uno del otro. Y eso nos llevaría a estimar erróneamente que el Tratamiento tiene un efecto sobre el Tamaño (U afecta al Tamaño y U se asocia con el Tratamiento cuando condicionamos sobre el Hongo), cuando el Tratamiento realmente no tiene un efecto sobre el Tamaño.

\section{Estructuras DAG básicas}
\subsection{Las tres estructuras DAG básicas}
Las estructuras DAG básicas con sus nombres se presentan en la Figura \ref{fig:plot_dag_struct}.
...
Reformulemos lo anterior. En el ejemplo de la cadena en la Figura \ref{fig:plot_dag_struct} a), si no condicionamos en Z, la tubería está abierta, por lo que X e Y se asocian\footnote{Etrictamente: "es muy probable que estén asociadas" ya que podría ocurrir que no lo estuvieran, pero sería raro.}. Pero si condicionamos en Z, cerramos la tubería, el flujo de asociación, y ahora X e Y son condicionalmente independientes dado Z. Lo mismo ocurre en la Figura \ref{fig:plot_dag_struct} b). Pero lo contrario sucede en la Figura \ref{fig:plot_dag_struct} c).

\begin{enumerate}[a)]

\item \textbf{Chain}: X e Y están asociados. Pero el condicionamiento sobre Z hará que X e Y sean independientes (no habrá asociación). 

\item \textbf{Fork}:  X e Y están asociados. Pero el condicionamiento sobre Z hará que X e Y sean independientes (no habrá asociación).

Este es el ejemplo habitual de \textit{confounding}, donde vemos asociación   debido a una causa común.

\item \textbf{Inverted fork with collider}: X e Y son independientes. Pero el condicionamiento en Z hará que X e Y se asocien. un modelo lineal, estarían correlacionadas. Asociación y no independencia son conceptos más generales que la correlación.
...
\subsection{Descendientes y ancestros en las estructuras DAG básicas}
Hemos modificado las estructuras básicas de la siguiente forma:
\begin{itemize}
\item En la fila superior, hemos añadido un descendiente de Z
\item En la fila inferior, hemos añadido un ancestro de Z
\end{itemize}
...
\begin{enumerate}[a)]
\setcounter{enumi}{3}
\item Si no se puede medir Z, pero se puede medir W, y W y Z están muy fuertemente asociados, condicionar sobre W puede ayudar a eliminar algún sesgo si se necesitan hacer X e Y condicionalmente independientes.  Porque el flujo a través de Z no ha sido interrumpido. Además:

  \begin{enumerate}[label=\arabic*.]
  \item X y W están asociados.
  \item Y y W están asociados.
  \item X y W son independientes si condicionamos en Z.
  \item Y y W son independient si condicionamos en Z.
  \end{enumerate}

\item Como arriba: condicionar en W no hace que X e Y sean independientes. Además:
...
\item Condicionar a W no hará que X e Y sean independientes. Observa también que \textbf{ahora Z es un colisionador con respecto a X y W}. Así que el condicionamiento sobre Z inducirá una asociación entre X y W. Además:
  \begin{enumerate}[label=\arabic*.]
  \item X y W son independientes.
  \item Y y W están asociados.
  \item X y W están asociados si condicionamos en Z.
  \item Y y W son independientes si condicionamos en Z.
  \end{enumerate}

\item Condicionar a W no hará que X e Y sean independientes. Además:
  \begin{enumerate}[label=\arabic*.]
  \item X y W están asociados.
  \item Y y W están asociados.
  \item X y W son independientes si condicionamos en Z.
...
\item Ahora Z es un colisionador con respecto a los tres pares de variables X, Y, W.
  \begin{enumerate}[label=\arabic*.]
    \item X e Y son independientes.
  \item X y W son independientes.
  \item Y y W son independientes.
  \item X y W están asociados si condicionamos en Z (Z es un collider).
  \item Y y W están asociados si condicionamos en Z. 
  \item X e Y están asociados si condicionamos en Z. 
  \end{enumerate}
\end{enumerate}

\section{Ejemplos adicionales}
\subsection{Variable collider pretratamiento que no debemos ajustar}
Supongamos las relaciones de la Figura \ref{fig:plot_dag_ptr}, y estamos interesados en el efecto de X sobre Y ($U_1$ y $U_2$ son dos variables diferentes no medidas). ¿Deberíamos ajustar por L, que es una variable previa al tratamiento?
...
Moraleja principal de esta historia: \textbf{ajustar por todas las posibles variables pre-tratamiento no siempre es una buena idea}.
Otra moraleja de la historia: \(U_c\) podría no ser una variable no medida. Tal vez esté ahí, en el conjunto de datos, pero no se ha utilizado. Si se incluyera en el modelo, habríamos bloqueado la puerta trasera.

Aún así, la moraleja principal de esta historia se mantiene: \textbf{Ajustar todas las variables posibles antes del tratamiento no siempre es una buena idea.}

\subsection{¿Debemos ajustar por la causa de la causa?}
Supongamos que las relaciones de la figura \ref{fig:plot_dag_zxy}, y estamos interesados en el efecto de X en Y. ¿Debemos ajustar para Z?
...
\subsection{La causa de la causa y amplificación del sesgo}
Consideremos ahora el DAG (U es una variable no medida) de la figura \ref{fig:plot_dag_bias_amp}:
...
\begin{quoting}
  ...
  Aunque está ampliamente aceptado que los niños nacidos de madres fumadoras tienen tienen un peso inferior al nacer y un mayor riesgo de mortalidad neonatal, se ha se ha sugerido que el efecto del tabaquismo materno se ve modificado por el que fumar es beneficioso para los bebés con BPN.
\end{quoting}

Este es otro caso de sesgo colisionador, en este caso estratificando sobre un efecto común. En la figura \ref{fig:bw-paradoja} hay dos posibles DAG causales que conducirían a esta paradoja.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.9\textwidth]{figs/bw-paradox-1-crop}
  \centering
  \includegraphics[width=0.9\textwidth]{figs/bw-paradox-2-crop}
  \caption{\label{fig:bw-paradox}. Two DAGs that would result in the
    birth-weight paradox. The
    presence or not of an arrow from LBW to Mortality is not the relevant feature.}
\end{figure}

¿Y qué es U y qué explicación intuitiva tiene este fenómeno? El bajo peso al nacer puede deberse a otras causas médicas distintas del tabaquismo. Una vez que sabemos que un bebé tiene bajo peso al nacer, si sabemos que es hijo de una madre fumadora, entonces es menos probable que el bebé tenga esas otras afecciones. De hecho, no necesitamos que BPN tenga ningún efecto sobre la mortalidad. Fíjate en el último DAG de la figura \ref{fig:bw-paradoja}: Fumar es perjudicial, pero otras causas de bajo peso al nacer pueden ser más perjudiciales que fumar. Así, entre los bebés con bajo peso al nacer, los de madres fumadoras presentan tasas de mortalidad más bajas. Pero decir a las embarazadas que fumen sería una muy mala idea.

\section{Relevancia en trabajo experimental/observacional}
