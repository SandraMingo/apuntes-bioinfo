...
\section{Symmetry and the paired t-test}
...
Es importante mencionar que los números hubiesen sido diferentes si en el código hubiésemos antepuesto B a A (\code{lm(y ~ B + A)}).
...
\item La función \texttt{aov} también puede utilizarse para ajustar modelos ANOVA. No la utilizamos para ajustar modelos de regresión. Y la mayoría de los modelos que se pueden ajustar con \texttt{aov} se pueden ajustar con \texttt{lm}. En lo que respecta a este curso, su sintaxis es la misma.
\item \texttt{Anova} y \texttt{anova} le dan tablas ANOVA de modelos ajustados con \texttt{lm}. La principal diferencia entre los dos es que \texttt{Anova} da, por defecto, lo que se llama sumas de cuadrados y pruebas de Tipo II, que utilizaremos la mayoría de las veces para tratar cuestiones sobre el orden de los factores. Todo esto se explicará en detalle.
\item Función \texttt{anova} también se puede utilizar para comparar modelos. Veremos ejemplos.
\item La función \texttt{summary} sobre un objeto ajustado con \texttt{aov} también le dará una tabla ANOVA. Rara vez usaremos esto (aunque podría ser el código que algunos menús en R commander realmente generan, y usted podría verlo en el código de otras personas).
...
\item Supongamos que en la muestra hay 10 mujeres que beben café y 10 hombres que no beben café. ¿Se puede decir algo sobre el efecto del sexo que no esté diciendo algo (o incluso todo) sobre el efecto del café?
\item Ahora supongamos que el diseño está perfectamente equilibrado: 5 mujeres que beben café, 5 mujeres que no beben café, 5 hombres que beben café, 5 hombres que no beben café. Si se dijera "he medido a una mujer", ¿se podría saber si también es bebedora de café o no?
\item Ahora, piensa en escenarios intermedios.
