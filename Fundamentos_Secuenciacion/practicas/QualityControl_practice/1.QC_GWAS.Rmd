...
plink --bfile --check-sex --out plink
...
```{bash}
grep "PROBLEM" plink.sexcheck | awk '{ADD}' > sex_discrepancy.txt

```

Then, apply `--remove` flag to remove individuals present in _sex_discrepancy.txt_ file.
```{bash}
plink --bfile --remove --make-bed --out HapMap_3_r3_4
...
```{bash}
plink --bfile --maf --make-bed --out HapMap_3_r3_6
```
