...
write_tsv(data.frame(selected_cells),
...
mart <- useEnsembl(biomart = "genes",
                   ...
                   version = 99) #la versión de la referencia depende del experimento 
...
# 34, se les va a poner un nombre único
...
table(rownames(sce) %in% genesMetadata$ensembl_gene_id)
...
seuratSC <- seuratSC %>%
  NormalizeData(
    ...
    normalization.method = "CLR",
    margin = 2) %>%
  ScaleData() #nunca se hace con toda la batería de la matriz de expresión
```

```{r}
rownames(seuratSC)
#devuelve los anticuerpos
```
...
```{r}
#BiocManager::install("scDblFinder", binary=TRUE)
#update? NO
library(scDblFinder)
DefaultAssay(seuratSC) <- "RNA" #en función de perfiles tradicionales de genes
...
```
No ha funcionado muy allá... 1300 genes que eran dobletes no se han caracterizado como tal. Por eso se hacen varias rondas de búsqueda de doublets.
...
Técnicamente ya hemos normalizado previamente, pero no pasa nada. Nos cargamos la normalización por pooles porque vamos a reemplazar la matriz.
...
seuratSC <- seuratSC %>%
  NormalizeData(
    assay = "RNA",
    normalization.method = "LogNormalize" #este es el básico
  ) %>%
  FindVariableFeatures(
    assay = "RNA",
    nfeatures = 2000, #2000 es un estándar, pero depende un poco de la complejidad del experimento. Si solo hay linfocitos T, la variabilidad será poca y se debería bajar el número para no introducir ruido de problemas técnicos
    selection.method = "vst", # or mvp, pero este es más manual al indicar el nivel de expresión y variabilidad
    verbose = F) %>%
  ScaleData() #importante hacer después de la selección de los genes

```

```{r}
seuratSC@assays$RNA@var.features
#lista de features

seuratSC@assays$RNA@scale.data
```

```{r}
# BiocManager::install('glmGamPoi', type="binary")
seuratSC <- seuratSC %>%
  SCTransform( #normaliza y selecciona variable genes transformando los datos
    assay = "RNA",
    conserve.memory = T,
...
```{r }
#listas de genes que intervienen en las fases S y G2M
...
seuratSC <- seuratSC %>%
 ...
 AddModuleScore(
   ...
   name = "CC.Score")
#clasifica las células en base a si están en la fase S o G2M por la lista de genes
#esto se suele hacer si se clusteriza un conjunto de células en proliferación y se quiere evitar que se clasifiquen por el ciclo celular en el que están
#el score se va a utilizar para eliminar ese efecto de células proliferativas vs no proliferativas
```

#### Compute Enrichment of CC genes in PC componets

Evaluar cómo de enriquecido está el componente de la PCA en genes proliferativos. Luego se quiere quitar esa componente del clustering para que no tenga efecto.

```{r}
# Get All PC Embeddings (coordenadas de la PCA)
...
df <- df[sample(nrow(df),size = min(1000,length(Cells(seuratSC)))),]
...
# Filter to those in our single cell object (quizás no tenemos todos los genes, y queremos solo aquellos que estén en nuestra lista de variable genes)
...
```
Como mucho se cogerían 20 componentes para tener toda la variabilidad necesaria. De ellas, se quitarían después aquellas que tengan mucha influencia del ciclo celular.

```{r}
seuratSC %>%
  ElbowPlot(ndims = 30,reduction = "ADTpca")
```
Aquí se usarían todos porque son poquitos anticuerpos.
...
```
Aquí se volverían a usar alrededor de 20 componentes.
...
seuratSC <- seuratSC %>%
  RunUMAP(
    assay = "RNA",
    reduction = "pca", #también se podría hacer sobre el grafo, pero depende de tener Python con una librería (reticulate para hacerlo desde R mismo pero con acceso a Python)
    dims = 1:16,
    min.dist = 0.3,
    spread = 1,
    n.neighbors = 30L,
    metric = "cosine", # euclidean para los grafos, y para UMAP valen los dos
...
```
La numeración es distinta para cada grafo (el 1 del RNAUMAP no es el 1 del SCTUMAP), y se da en función del tamaño del cluster.
...
```{r}
library(SingleR) #librería para clasificar células en base a perfiles ya conocidos (bases de datos en R)
...
cellannot <- SingleR(
  ...
  , ref = dbImm #matriz con el tipo celular y los distintos perfiles en base al tejido
...
Misma célula con distinta información: RNA y anticuerpos. Se genera un grafo de neighbours en base a los vecinos de uno y de otro. 
```{r}
seuratSC <- seuratSC %>%
  FindMultiModalNeighbors(
    reduction.list = list("pca", "ADTpca"),  #se juntan anticuerpos y genes
    dims.list = list(1:20, 1:20), #componentes a usar
...
```{r}
DimPlot(seuratSC, reduction = 'wnn.umap', group.by = "wsnn_res.0.5", label = TRUE, repel = TRUE, label.size = 2.5,cols = color.list)
```
...
myMarkers <- seuratSC %>%
  FindAllMarkers(
    assay = "RNA",
    slot = "data",
    min.pct = 0.3, #solo se evalúan las células que compartan el 30%
    logfc.threshold = 0.5,
    test.use = "wilcox" #es el que está por defecto y es seguro al ser no paramétrico
  )
```
...
```
Es posible que el cluster 7 sean dobletes o células muertas porque no hay nada que destaque. 
Los clústers 0 y 1 son similares en su patrón, por lo que se podrían fusionar.

```{r}
DimPlot(seuratSC, reduction = "RNAumap", group.by = c("RNA_snn_res.0.5", "HTO_classification.global"))
```


Efectivamente se ve que donde se encuentra el cluster 7 se encuentra un grupo de dobletes.